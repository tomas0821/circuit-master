...
def _tier(obj_type, n):
    ...
    if obj_type == "capacitance":
        ...
        return tier, color
    if n == 4:
        return tier, '#6a1b9a'   # purple
    if n == 3:
        return tier, '#e65100'   # orange
    return         tier, '#2e7d32'   # green


def draw_deck_card(ax, route_desc, target_req, points, obj_type="resistance", n=2):
    """
    Render one deck card on `ax`.
    Data coords: xlim=(0,10), ylim=(0,14)  →  isotropic at 2.5" × 3.5"
    """
    ax.set_xlim(0, 10)
    ax.set_ylim(0, 14)
    ax.axis('off')
    ...
    # write-in line for students to fill in the equivalent value
    ...
    # component count (top-right corner)
    ...
    # component icons (left) — small resistor/cap symbols
    icon_lw = 1.4
    icon_y = 1.4
    spacing = 1.5
    x_start = 0.9
...
def draw_card_back(ax):
    """Render the card back: logo centred, circuit-trace border, teal/orange palette."""
    ax.set_xlim(0, 10)
    ax.set_ylim(0, 14)
    ax.axis('off')
    ...
    # ── circuit-trace corner decorations ──────────────────────────────────────
    def corner(cx, cy, sx, sy):
        """Draw an L-shaped PCB trace + dot at one corner."""
        lx, ly = cx + sx * 1.2, cy + sy * 1.2
    ...
    if os.path.exists(LOGO_PATH):
        ...
        logo_w = 7.6          # data units wide
        logo_h = logo_w * h_px / w_px
        logo_x0 = (10 - logo_w) / 2
        logo_y0 = 4.0
        # white backing so logo blends cleanly regardless of card BG color
    ...
    ax.text(5, 1.55, "OBJECTIVE CARD",
            ha='center', va='center', fontsize=9, fontweight='bold',
            color='white', zorder=6)
    ax.text(5, 0.75, "An Educational Electronics Game",
            ha='center', va='center', fontsize=6, color='white', alpha=0.85, zorder=6)


def draw_component_back(ax):
    """Card back for component cards — charcoal/amber palette, 'COMPONENT CARD' label."""
    ax.set_xlim(0, 10)
    ax.set_ylim(0, 14)
    ax.axis('off')
...
# ── layout constants ──────────────────────────────────────────────────────────
CARD_W = 2.5    # inches — standard playing card
CARD_H = 3.5    # inches

# ── A4 printable PDF (3×3 cards per page) ────────────────────────────────────
import math
...
COLS_PAGE   = 3
ROWS_PAGE   = 3
PER_PAGE    = COLS_PAGE * ROWS_PAGE   # 9 cards per page

# margins and gaps chosen so 3×2.5 = 7.5" and 3×3.5 = 10.5" centre nicely on A4
GAP_X  = 0.10
GAP_Y  = 0.10
TITLE_H = 0.38
GRID_W  = COLS_PAGE * CARD_W + (COLS_PAGE - 1) * GAP_X   # 7.7"
GRID_H  = ROWS_PAGE * CARD_H + (ROWS_PAGE - 1) * GAP_Y   # 10.7"
MARGIN_X = (A4_W - GRID_W) / 2   # ~0.285"
MARGIN_Y = (A4_H - GRID_H - TITLE_H) / 2   # ~0.145"

n_pages = math.ceil(len(CARDS) / PER_PAGE)

def _add_crop_marks(fig, left_in, bottom_in, w_in, h_in, tick=0.10, lw=0.4):
    """Draw small crop-mark ticks at the four corners of a card (figure coords)."""
    def fi(x): return x / A4_W   # inch → figure fraction x
    def fj(y): return y / A4_H   # inch → figure fraction y
...
PDF_DPI = 150   # 150 dpi is sharp enough for cards on a home printer

# ── fronts PDF ────────────────────────────────────────────────────────────────
with PdfPages('deck_a4_fronts.pdf') as pdf:
...
print(f"Saved deck_a4_fronts.pdf  ({n_pages} pages)")

# ── backs PDF (columns mirrored for duplex long-edge flip) ───────────────────
with PdfPages('deck_a4_backs.pdf') as pdf:
    for page in range(n_pages):
        fig = plt.figure(figsize=(A4_W, A4_H), facecolor='white')
        ...
        for i in range(n_this):
            row = i // COLS_PAGE
            col = (COLS_PAGE - 1) - (i % COLS_PAGE)   # mirror for duplex
            left_in   = MARGIN_X + col * (CARD_W + GAP_X)
            bottom_in = MARGIN_Y + (ROWS_PAGE - 1 - row) * (CARD_H + GAP_Y)
            ax = fig.add_axes([left_in / A4_W, bottom_in / A4_H,
                               CARD_W / A4_W,  CARD_H / A4_H])
...
print(f"Saved deck_a4_backs.pdf   ({n_pages} pages)")

# ── overview PNG (8×4 grid, for reference) ───────────────────────────────────
N_COLS = 8
N_ROWS = 4
MARGIN = 0.35
GAP    = 0.12
...
fig.savefig('deck_objective_cards.png', dpi=200, bbox_inches='tight', facecolor='#eeeeee')
plt.close(fig)
print(f"Saved deck_objective_cards.png  ({N_COLS}×{N_ROWS} overview)")

# ── interleaved PDF (front1, back1, front2, back2, …) for easy duplex print ──
...
n = _interleave_pdfs('deck_a4_fronts.pdf', 'deck_a4_backs.pdf', 'deck_a4_print.pdf')
print(f"Saved deck_a4_print.pdf  ({n} pages, interleaved front/back)")
...
# Expand to full print quantity so the PDF is ready to send directly to the printer
COMPONENT_CARDS = [card for card in _COMPONENT_TYPES for _ in range(card['count'])]

def _draw_wire_symbol(ax, xc, yc, color, lw=3):
    """Arc representing a physical jumper wire on a breadboard."""
...
def draw_component_card(ax, card_type, value, unit, color, count, label):
    ...
    # ── placement rule ────────────────────────────────────────────────────────
    rules = {
        'wire':      'Connect adjacent columns\nin the same row',
        'resistor':  'Place between two adjacent\nrows in the same column',
        'capacitor': 'Place between two adjacent\nrows in the same column',
    }
...
# ── component fronts PDF ─────────────────────────────────────────────────────
with PdfPages('components_a4_fronts.pdf') as pdf:
...
print(f"Saved components_a4_fronts.pdf  ({n_pages_c} page{'s' if n_pages_c > 1 else ''})")

# ── component backs PDF ──────────────────────────────────────────────────────
with PdfPages('components_a4_backs.pdf') as pdf:
    for page in range(n_pages_c):
...
print(f"Saved components_a4_backs.pdf   ({n_pages_c} page{'s' if n_pages_c > 1 else ''})")

# ── component interleaved print PDF ─────────────────────────────────────────
n = _interleave_pdfs('components_a4_fronts.pdf', 'components_a4_backs.pdf',
                     'components_a4_print.pdf')
print(f"Saved components_a4_print.pdf  ({n} pages, interleaved front/back)")
